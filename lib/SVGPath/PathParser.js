/**
 * SVG Path high level tokens
 *
 * @typedef {Object} PathToken
 * @prop {String} type - The type of token.
 *   It can be either `wsp`, `error`, or the character representing a valid
 *   command as define per the SVG path grammar.
 * @prop {Number} index - The index of the first character
 *   of the token within the path.
 * @prop {Number} Length - The length of the token.
 * @prop {Array<Number>} [numbers] - For command with numbers, it is all the
 *   valid numbers extracted from the associated command
 * @prop {String} [msg] - For error token, a string describing the error
 */
